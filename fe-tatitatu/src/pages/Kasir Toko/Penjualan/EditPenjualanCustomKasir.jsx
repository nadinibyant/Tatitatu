...
const EditPenjualanCustomKasir = () => {
            ...
            const newDataProduk = [
                {
                    ...
                    data: existingData.data_produk.map((item, index) => ({
                        ...
                        "Nama Barang": (
                            <InputDropdown
                                showRequired={false}
                                options={dataBarang.map(prod => ({ id: prod.id, label: prod.name }))}
                                value={item["Nama Barang"]}
                                onSelect={(selectedItem) => handleProductSelect(0, item.id, selectedItem)}
                            />
                        ),
                        ...
                        "Kuantitas": (
                            <Input
                                ...
                                onChange={(newCount) => handleQuantityChange(0, item.id, newCount, item["Harga Satuan"])}
                            />
                        ),
                ...
                {
                    nama: 'Rincian Biaya',
                    data: existingData.rincian_biaya.map((item, index) => ({
                        id: item.id,
                        No: index + 1,
                        "Nama Biaya": (
                            <Input
                                ...
                                onChange={(value) => handleNamaBiayaChange(1, item.id, value)}
                            />
                        ),
                        "Jumlah Biaya": (
                            <Input
                                ...
                                onChange={(value) => handleJumlahBiayaChange(1, item.id, value)}
                            />
                        ),
                        rawTotalBiaya: item["Jumlah Biaya"],
                        Aksi: (
                            <button
                                className="text-red-500 hover:text-red-700"
                ...
                {
                    nama: 'Packaging',
                    data: existingData.data_packaging.map((item, index) => ({
                        id: item.id,
                        No: index + 1,
                        "Foto Produk": (
                            <img
                        ...
                        "Nama Barang": (
                            <InputDropdown
                                showRequired={false}
                                options={dataPackaging.map(pack => ({ id: pack.id, label: pack.name }))}
                                value={item["Nama Barang"]}
                                onSelect={(selectedItem) => handleProductSelect(2, item.id, selectedItem)}
                            />
                        ),
                        "Harga Satuan": `Rp${item["Harga Satuan"].toLocaleString('id-ID')}`,
                        "Kuantitas": (
                            <Input
                                ...
                                type="number"
                                value={item.kuantitas}
                                onChange={(newCount) => handleQuantityChange(2, item.id, newCount, item["Harga Satuan"])}
                            />
                        ),
                        "Total Biaya": `Rp${item["Total Biaya"].toLocaleString('id-ID')}`,
                        rawTotalBiaya: item["Total Biaya"],
    ...
    const handleTambahSubmit = (e) => {
        ...
        try {
            ...
            const updatedData = {
                ...
                note,
                data_produk: dataProduk[0].data.map(item => ({
                    id: item.id,
                    "Foto Produk": typeof item["Foto Produk"] === 'string' ? item["Foto Produk"] : item["Foto Produk"].props.src,
                    "Nama Barang": typeof item["Nama Barang"] === 'string' ? item["Nama Barang"] : item["Nama Barang"].props.value,
                    "Harga Satuan": parseInt(item["Harga Satuan"].replace(/[^0-9]/g, '')),
                    kuantitas: item.quantity || item.kuantitas,
                    "Total Biaya": item.rawTotalBiaya
                })),
                rincian_biaya: dataProduk[1].data.map(item => ({
                    id: item.id,
                    "Nama Biaya": typeof item["Nama Biaya"] === 'string' ? item["Nama Biaya"] : item["Nama Biaya"].props.value,
                    "Jumlah Biaya": item.rawTotalBiaya
                })),
                data_packaging: dataProduk[2].data.map(item => ({
                    id: item.id,
                    "Foto Produk": typeof item["Foto Produk"] === 'string' ? item["Foto Produk"] : item["Foto Produk"].props.src,
                    "Nama Barang": typeof item["Nama Barang"] === 'string' ? item["Nama Barang"] : item["Nama Barang"].props.value,
                    "Harga Satuan": parseInt(item["Harga Satuan"].replace(/[^0-9]/g, '')),
    ...
    const handleProductSelect = (tableIndex, itemId, selectedItem) => {
        const updatedDataTables = [...dataProduk];
        const itemIndex = updatedDataTables[tableIndex].data.findIndex(
            (data) => data.id === itemId
        );
    
        if (itemIndex !== -1) {
            const selectedProduct = tableIndex === 2
                ? dataPackaging.find(pack => pack.name === selectedItem.label)
                : dataBarang.find(product => product.name === selectedItem.label);
            
            if (selectedProduct) {
                const currentQuantity = updatedDataTables[tableIndex].data[itemIndex].quantity || 0;
                const newTotalBiaya = selectedProduct.price * currentQuantity;
                
                updatedDataTables[tableIndex].data[itemIndex] = {
                    ...updatedDataTables[tableIndex].data[itemIndex],
                    "Nama Barang": (
                        <InputDropdown
                            showRequired={false}
                            options={tableIndex === 2 
                                ? dataPackaging.map(pack => ({ id: pack.id, label: pack.name }))
                                : dataBarang.map(prod => ({ id: prod.id, label: prod.name }))
                            }
                            value={selectedItem.label}
                            onSelect={(newSelection) => handleProductSelect(tableIndex, itemId, newSelection)}
                        />
                    ),
                    "Harga Satuan": `Rp${selectedProduct.price.toLocaleString('id-ID')}`,
    ...
    const handleQuantityChange = (tableIndex, itemId, newCount, currentPrice) => {
        const updatedTablesCopy = [...dataProduk];
        const itemIndex = updatedTablesCopy[tableIndex].data.findIndex(
            (row) => row.id === itemId
        );
    
        if (itemIndex !== -1) {
            updatedTablesCopy[tableIndex].data[itemIndex].quantity = newCount;
            const newTotal = currentPrice * Number(newCount);
            updatedTablesCopy[tableIndex].data[itemIndex].rawTotalBiaya = newTotal;
            updatedTablesCopy[tableIndex].data[itemIndex]["Total Biaya"] = `Rp${newTotal.toLocaleString('id-ID')}`;
            setDataProduk(updatedTablesCopy);
        }
    };
