...
export default function EditPemasukan() {
    ...
    const tableData = tableRows.map((row, index) => ({
        ...
        jumlahPemasukan: (
            <Input
                type="number"
                value={row.jumlahPemasukan}
                onChange={(value) => handleRowChange(row.id, 'jumlahPemasukan', value)}
                width="w-40"
                showRequired={false}
                required={false}
            />
    ...
    const tableHeaders = [
        ...
        { key: 'jumlahPemasukan', label: 'Jumlah Pemasukan' },
        { key: 'aksi', label: 'Aksi' }
    ];

    const calculateTotal = () => {
        return tableRows.reduce((sum, row) => {
            const amount = parseInt(row.jumlahPemasukan) || 0;
            return sum + amount;
        }, 0);
    };
    ...
    const handleSave = () => {
        ...
        const updatedData = {
            ...
            tableData: tableRows.map(row => ({
                ...
                toko: row.toko,
                cabang: row.cabang,
                jumlahPemasukan: row.jumlahPemasukan
            }))
        };
