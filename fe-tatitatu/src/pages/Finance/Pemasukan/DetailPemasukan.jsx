...
export default function DetailPemasukan() {
    ...
    useEffect(() => {
        ...
        fetchPemasukan(nomor);
    }, [nomor, navigate]);

    const fetchPemasukan = async (id) => {
        try {
            setIsLoading(true);
            ...
            if (response.data.success) {
                ...
                const subtotal = responseData.deskripsi_pemasukan.reduce(
                    (sum, item) => sum + (item.jumlah_pemasukan || 0), 0
                );

                const potongan = subtotal - responseData.total;

                setData({
                    nomor: responseData.pemasukan_id,
                    tanggal: responseData.tanggal,
                    kategori: responseData.kategori_pemasukan,
                    cash_or_non: responseData.cash_or_non,
                    metode: responseData.metode || '-',
                    data_pemasukan: responseData.deskripsi_pemasukan.map(item => ({
                        Deskripsi: item.deskripsi,
                        Toko: item.toko,
                        Cabang: item.cabang || '-',
                        Pemasukan: item.jumlah_pemasukan
                    })),
                ...
                setError(response.data.message || 'Gagal mengambil data pemasukan');
            }
        } catch (error) {
            console.error('Error fetching pemasukan data:', error);
            setError('Terjadi kesalahan saat mengambil data pemasukan');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleEdit = () => {
        navigate(`/pemasukan/edit/${data.nomor}`, { 
            state: { 
                nomor: data.nomor,
    ...
    // const handleEdit = () => {
    //     navigate(`/pemasukan/edit/${data.nomor}`, { state: { nomor: data.nomor } });
    // };
    ...
    const handleConfirmDel = async () => {
        try {
            setIsLoading(true);
            const response = await api.delete(`/pemasukan/${data.nomor}`);
            ...
            if (response.data.success) {
                ...
                setError(response.data.message || 'Gagal menghapus data pemasukan');
            }
        } catch (error) {
            console.error('Error deleting pemasukan:', error);
            setError('Terjadi kesalahan saat menghapus data pemasukan');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <LayoutWithNav>
                <div className="p-5">
                    <AlertError
                        title="Error"
                        description={error}
                        confirmLabel="Kembali ke Daftar"
                        onConfirm={() => navigate('/pemasukan')}
                    />
                </div>
            </LayoutWithNav>
        );
                        ...
                        <div className="w-1/2 lg:w-1/3 space-y-4 text-sm">
                            {/* {data.potongan > 0 && (
                                <div className="flex justify-between">
                                    ...
                                    <p>{formatRupiah(data.potongan)}</p>
                                </div>
                            )} */}
                            
                            <div className="flex justify-between pb-2 pt-2">
                                <p className="font-bold">Total</p>
                                <p className="font-bold">{formatRupiah(data.total)}</p>
                            </div>
                        </div>
                    </section>
                </section>
